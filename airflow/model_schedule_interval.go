...
func (dst *ScheduleInterval) UnmarshalJSON(data []byte) error {
	...
	// check if the discriminator value is 'CronExpression'
	if jsonDict["__type"] == "CronExpression" {
		// try to unmarshal JSON data into CronExpression
		err = json.Unmarshal(data, &dst.CronExpression);
		if err == nil {
			jsonCronExpression, _ := json.Marshal(dst.CronExpression)
			if string(jsonCronExpression) == "{}" { // empty struct
				dst.CronExpression = nil
			} else {
				return nil // data stored in dst.CronExpression, return on the first match
			}
		} else {
			dst.CronExpression = nil
		}
	...
	// check if the discriminator value is 'RelativeDelta'
	if jsonDict["__type"] == "RelativeDelta" {
		// try to unmarshal JSON data into RelativeDelta
		err = json.Unmarshal(data, &dst.RelativeDelta);
		if err == nil {
			jsonRelativeDelta, _ := json.Marshal(dst.RelativeDelta)
			if string(jsonRelativeDelta) == "{}" { // empty struct
				dst.RelativeDelta = nil
			} else {
				return nil // data stored in dst.RelativeDelta, return on the first match
			}
		} else {
			dst.RelativeDelta = nil
		}
	...
	// check if the discriminator value is 'TimeDelta'
	if jsonDict["__type"] == "TimeDelta" {
		// try to unmarshal JSON data into TimeDelta
		err = json.Unmarshal(data, &dst.TimeDelta);
		if err == nil {
			jsonTimeDelta, _ := json.Marshal(dst.TimeDelta)
			if string(jsonTimeDelta) == "{}" { // empty struct
				dst.TimeDelta = nil
			} else {
				return nil // data stored in dst.TimeDelta, return on the first match
			}
		} else {
			dst.TimeDelta = nil
		}
	...
	return fmt.Errorf("Data failed to match schemas in anyOf(ScheduleInterval)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *ScheduleInterval) MarshalJSON() ([]byte, error) {
	if src.CronExpression != nil {
		return json.Marshal(&src.CronExpression)
	}

	if src.RelativeDelta != nil {
		return json.Marshal(&src.RelativeDelta)
	}

	if src.TimeDelta != nil {
		return json.Marshal(&src.TimeDelta)
	}
